...
const BulkDietLogs = () => {
  ...
  const nutritionGoals = [
    { label: 'Calories', value: userGoals?.target_calories || 2415, current: 0, color: 'text-primary' },
    { label: 'Protein', value: userGoals?.protein_grams || 150, unit: 'g', current: 0, color: 'text-green-500' },
    { label: 'Carbs', value: userGoals?.carbs_grams || 280, unit: 'g', current: 0, color: 'text-blue-500' },
    { label: 'Fat', value: userGoals?.fat_grams || 70, unit: 'g', current: 0, color: 'text-yellow-500' },
  ];
          ...
          <div className="flex items-center justify-between mb-12">
            <div>
              <h1 className="text-4xl font-bold mb-2">
                <span className="text-gradient">Bulk Diet Plan</span>
              </h1>
              <p className="text-muted-foreground text-lg">
                Track your meals with increased calories for muscle gain
              </p>
              {isReadOnly && (
              ...
              >
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-background/30 rounded-lg border border-border/30 hover:border-primary/50 transition-all cursor-pointer">
                    <h4 className="font-semibold text-sm mb-1">Steak with Sweet Potato</h4>
                    <p className="text-xs text-muted-foreground mb-2">High protein & carbs</p>
                    <div className="flex gap-2 text-xs">
                      <span className="bg-primary/20 px-2 py-1 rounded">620 cal</span>
                      <span className="bg-green-500/20 px-2 py-1 rounded">50g protein</span>
                    </div>
                  </div>
                  
                  <div className="p-3 bg-background/30 rounded-lg border border-border/30 hover:border-primary/50 transition-all cursor-pointer">
                    <h4 className="font-semibold text-sm mb-1">Pasta with Chicken</h4>
                    <p className="text-xs text-muted-foreground mb-2">Carb-rich meal</p>
                    <div className="flex gap-2 text-xs">
                      <span className="bg-primary/20 px-2 py-1 rounded">580 cal</span>
                      <span className="bg-blue-500/20 px-2 py-1 rounded">75g carbs</span>
                    </div>
                  </div>
                  
                  <div className="p-3 bg-background/30 rounded-lg border border-border/30 hover:border-primary/50 transition-all cursor-pointer">
                    <h4 className="font-semibold text-sm mb-1">Protein Smoothie Bowl</h4>
                    <p className="text-xs text-muted-foreground mb-2">Nutrient-dense snack</p>
                    <div className="flex gap-2 text-xs">
                      <span className="bg-primary/20 px-2 py-1 rounded">450 cal</span>
                      <span className="bg-yellow-500/20 px-2 py-1 rounded">18g fat</span>
                    </div>
                  </div>
                </div>
              </motion.div>
                ...
                {mealTypes.map((meal, index) => (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <input
                        type="text"
                        placeholder="e.g., Beef, Rice & Avocado"
